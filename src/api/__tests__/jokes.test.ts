...
describe('Jokes API', () => {
  ...
  describe('fetchRandomJoke', () => {
    ...
    it('fetches a random joke with category', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://api.chucknorris.io/jokes/random?category=dev')
    })
  ...
  describe('searchJokes', () => {
    ...
    it('searches jokes successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://api.chucknorris.io/jokes/search?query=test')
    })
    ...
    it('handles fetch error', async () => {
      ...
      await expect(searchJokes('test')).rejects.toThrow('Failed to search jokes')
    })
  })
